                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Syarat Nilai</th>
                                <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Kuota</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Jadwal Pelaksanaan</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($slots as $slot)
                                <tr>
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-medium text-gray-900">{{ $slot->company->name }}</div>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        <div>Total SMART: &ge; {{ $slot->min_total_score }}</div>
                                        <div>Absensi: &ge; {{ $slot->min_absensi_score }}</div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <span class="text-sm font-bold text-indigo-600">{{ $slot->quota }} Siswa</span>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        <div class="font-medium text-gray-900">Mulai: {{ \Carbon\Carbon::parse($slot->start_date)->translatedFormat('d M Y') }}</div>
                                        <div class="text-red-600">Tarik: {{ \Carbon\Carbon::parse($slot->end_date)->translatedFormat('d M Y') }}</div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
